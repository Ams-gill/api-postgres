...
const divRegister = document.getElementById("div-create"); 
...
let showRegisterPanel = (bShow) => {
    bShow ? divRegister.style.display = "flex" : divRegister.style.display = "none";
  }
...
formRegister.onsubmit = async e => {
    e.preventDefault();
    const registerDetails = await register({ name: formRegister.name.value, email: formRegister.email.value, password: formRegister.password.value });
    console.log(registerDetails);
    if (registerDetails.error) {
        pStatus.innerText = registerDetails.error;
        return;
      }
      pStatus.innerHTML = `Register Successful!`;
